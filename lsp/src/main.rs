...
use lsp::{
    ...
    nrs_lang::{Ast, ImCompleteSemanticToken, ParserResult, parse, type_inference},
    semantic_analyze::{IdentType, Semantic, analyze_program},
    semantic_token::LEGEND_TYPE,
    span::Span,
};
use ropey::Rope;
        ...
        let inlay_hint_list = hashmap
            ...
            .map(|(k, v)| {
                (
                    ...
                    k.end,
                    match v {
                        lsp::nrs_lang::Value::Null => "null".to_string(),
                        lsp::nrs_lang::Value::Bool(_) => "bool".to_string(),
                        lsp::nrs_lang::Value::Num(_) => "number".to_string(),
                        lsp::nrs_lang::Value::Str(_) => "string".to_string(),
                    },
                )
            })
            .filter_map(|item| {
        ...
        let completions = || -> Option<Vec<CompletionItem>> {
            ...
            for (_, item) in completions {
                match item {
                    ...
                    lsp::completion::ImCompleteCompletionItem::Function(name, args) => {
                        ret.push(CompletionItem {
                            label: name.clone(),
