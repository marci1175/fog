    ...
    while token_idx < tokens.len() {
        ...
        match current_token {
            ...
            Token::UnparsedLiteral(raw_string) => {
                // The type mismatch will be caught in this function
                let parsed_value =
                    unparsed_const_to_typed_literal(raw_string.clone(), variable_type)?;

                // Initialize parsed token with a value
                if parsed_token.is_none() {
                    parsed_token = Some(ParsedToken::Literal(parsed_value));
                }
            }
            ...
            Token::Literal(literal) => {
                ...
                // Get the type discriminant of the type
                let literal_type = literal.discriminant();

                let next_token = tokens.get(token_idx + 1);

                // If the next token is an `As`
                if let Some(Token::As) = next_token {
                    if let Some(Token::TypeDefinition(target_type)) = tokens.get(token_idx + 2) {
                        literal = convert_as(literal.clone(), *target_type)?;
                    } else {
                        return Err(ParserError::SyntaxError(
                ...
                // Return a type error if the variable's and the literal's types mismatch and there is no `As` keyword
                else if literal_type != variable_type {
                    return Err(ParserError::TypeError(literal_type, variable_type).into());
                }

                // Initialize parsed token with a value.
                if parsed_token.is_none() {
                    parsed_token = Some(ParsedToken::Literal(literal.clone()));
                }
            }
